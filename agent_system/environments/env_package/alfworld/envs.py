...
class AlfworldWorker:
    ...
    def ready(self):
        """Confirm actor construction before starting the next init batch."""
        return True
...
class AlfworldEnvs(gym.Env):
    def __init__(self, alf_config_path, seed, env_num, group_n, resources_per_worker, is_train=True, env_kwargs={}):
        ...
        # TextWorld actor construction is expensive. Creating all actors at once
        # can trigger a Ray worker-registration storm, so start bounded batches
        # and wait for each batch to become ready before continuing.
        env_worker = ray.remote(**resources_per_worker)(AlfworldWorker)
        init_batch_size = max(1, int(os.getenv("ALFWORLD_INIT_BATCH_SIZE", "8")))
        self.workers = []
        for start in range(0, self.num_processes, init_batch_size):
            batch_workers = [
                env_worker.remote(config, seed + (i // self.group_n), base_env)
                for i in range(start, min(start + init_batch_size, self.num_processes))
            ]
            ray.get([worker.ready.remote() for worker in batch_workers])
