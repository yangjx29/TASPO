...
def adjust_batch(config, data: DataProto, mode="copy", mark_padding: bool = False) -> DataProto:
    ...
    if "multi_modal_inputs" in data.non_tensor_batch:
        ...
        if mark_padding:
            # TASPO uses zero-mass copied rows, so it can safely pad every PPO
            # minibatch to the same size and avoid an overweight final minibatch.
    ...
    if mark_padding:
        data.non_tensor_batch["is_padding_row"] = np.zeros(bs, dtype=bool)
    ...
    if mode == "delete":
        ...
        if mark_padding:
            dup_proto.non_tensor_batch["is_padding_row"] = np.ones(to_add, dtype=bool)
