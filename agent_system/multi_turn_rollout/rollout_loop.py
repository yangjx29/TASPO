...
def _trace_text(value) -> str:
    """Convert an environment state into a compact, JSON-safe trace string."""
    if value is None:
        return ""
...
def _trace_values(values, batch_size: int) -> np.ndarray:
    if values is None:
        return np.array([""] * batch_size, dtype=object)
    return np.array([_trace_text(values[i]) for i in range(batch_size)], dtype=object)


def _environment_action(model_response: str, projected_action: str) -> str:
    """Return the executable action span rather than the model's reasoning text."""
    for tag in ("action", "search", "answer"):
        ...
        if match:
            ...
            return f"<{tag}>{content}</{tag}>" if tag in {"search", "answer"} else content
    return str(projected_action)
...
class TrajectoryCollector:
    ...
    def preprocess_single_sample(
        ...
        # Apply chat template
        prompt_with_chat_template = self.tokenizer.apply_chat_template(chat, add_generation_prompt=True, tokenize=False, **apply_chat_template_kwargs)
        ...
        for bs in range(batch_size):
            ...
            for step_idx, data in enumerate(total_batch_list[bs]):
                ...
                if data["active_masks"]:
                    ...
                    # A verified per-trajectory outcome. Unlike the aggregate
                    # success-rate metrics below, this value is safe to use for
                    # selecting successful sibling trajectories.
                    data["verified_success"] = float(success["success_rate"][bs])
                    # success_rate
    ...
    def vanilla_multi_turn_loop(
        ...
        for _step in range(self.config.env.max_steps):
            ...
            pre_observations = _trace_values(obs.get("anchor"), batch_size)
            env_tasks = getattr(envs, "tasks", None)
            task_texts = _trace_values(env_tasks, batch_size)
            ...
            model_responses = self.tokenizer.batch_decode(batch.batch["responses"], skip_special_tokens=True)
            text_actions = list(model_responses)
            ...
            batch.non_tensor_batch["task_text"] = task_texts
            batch.non_tensor_batch["pre_observation"] = pre_observations
            batch.non_tensor_batch["post_observation"] = _trace_values(next_obs.get("anchor"), batch_size)
            batch.non_tensor_batch["model_response_text"] = np.array(model_responses, dtype=object)
            batch.non_tensor_batch["action_text"] = np.array(
                [_environment_action(model_response, projected_action) for model_response, projected_action in zip(model_responses, text_actions)],
                dtype=object,
            )
            batch.non_tensor_batch["env_done"] = torch_to_numpy(dones, is_object=True)
