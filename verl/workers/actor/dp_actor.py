...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        if self.config.get("use_taspo_trajectory_balance", False):
            select_keys.append("taspo_trajectory_balance")
        batch = data.select(batch_keys=select_keys).batch
        has_multi_modal_inputs = "multi_modal_inputs" in data.non_tensor_batch.keys()
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    row_balance = data.get("taspo_trajectory_balance")
                    if row_balance is not None:
                        row_balance = row_balance.unsqueeze(-1)
                    ...
                    if entropy_coeff != 0:
                        entropy_for_loss = entropy if row_balance is None else entropy * row_balance
                        entropy_loss = agg_loss(loss_mat=entropy_for_loss, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
                    ...
                    if self.config.use_kl_loss:
                        ...
                        kld_for_loss = kld if row_balance is None else kld * row_balance
                        kl_loss = agg_loss(loss_mat=kld_for_loss, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)
